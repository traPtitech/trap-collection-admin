...
<template>
  <Listbox v-model="selectedItem" class="w-64">
    <div margin="t-1" position="relative">
      ...
      >
        <ListboxOptions
          class="rounded-md bg-true-gray-600 shadow-lg mt-1 w-full max-h-60 py-1 pl-0 ring-true-gray-900 ring-1 ring-opacity-5 overflow-auto sm:text-sm focus:outline-none"
        >
          <ListboxOption
            v-for="item in items"
            :key="item.name"
          ...
          >
            <li
              class="cursor-default list-none py-2 pr-4 pl-10 relative select-none"
            ...
            >
              ...
              <span
                v-if="selected"
                class="flex pl-3 inset-y-0 left-0 text-teal-300 items-center absolute"
              >
